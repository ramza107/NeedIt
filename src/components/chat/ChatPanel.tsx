...
export function ChatPanel({
  ...
  return (
    <div className="flex flex-col h-[500px] rounded-2xl border border-border bg-white">
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              {msg.is_system ? (
                <div className="rounded-full bg-muted-bg px-4 py-1.5 text-xs text-muted">
                  {msg.content}
                </div>
              ) : (
                <>
                  ...
                  <div className={`max-w-[70%] ${isOwn ? 'text-right' : ''}`}>
                    ...
                    <p className="text-xs text-muted mt-1">{formatRelativeTime(msg.created_at)}</p>
                  </div>
                </>
              )}
            </div>
